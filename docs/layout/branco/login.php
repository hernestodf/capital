...
<head>
  ...
  <style>
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    /* Toggle lembrar */
    .login-remember{display:flex;align-items:center;gap:10px;font-size:13.5px;color:var(--text-2);cursor:pointer}
    .tog-s{position:relative;width:40px;height:22px;flex-shrink:0}
    .tog-s input{display:none}
    .tog-s .tog-track{position:absolute;inset:0;border-radius:11px;background:var(--bg-surface);border:1.5px solid var(--bg-border);cursor:pointer;transition:all .2s;display:block}
    .tog-s .tog-thumb{position:absolute;top:3px;left:3px;width:14px;height:14px;border-radius:50%;background:var(--text-4);pointer-events:none;transition:transform .22s cubic-bezier(.34,1.56,.64,1),background .2s}
    .tog-s input:checked~.tog-track{background:var(--neon-cyan);border-color:var(--neon-cyan)}
    .tog-s input:checked~.tog-thumb{transform:translateX(18px);background:#fff;box-shadow:0 2px 10px var(--neon-cyan-glow)}

    /* Lembrar + Esqueceu */
    .login-options{display:flex;align-items:center;justify-content:space-between;margin-bottom:28px}
    ...
    /* Rodape */
    .login-footer{margin-top:32px;text-align:center;font-size:13px;color:var(--text-4)}
    .login-footer a{color:var(--neon-cyan);font-weight:600;text-decoration:none}
...
<div class="login-page">
  ...
  <div class="login-brand">
    <div class="login-brand-content">
      ...
      <h1>Sis<span>Loc</span> v3.0</h1>
      <p>Sistema de gestao de locacao de equipamentos. Gerencie contratos, estoque e clientes em uma unica plataforma.</p>
    ...
    <div class="login-brand-ver">SisLoc v3.0 · White Rabbit · Design System 3.0</div>
  </div>
  ...
  <div class="login-form-side">
    <div class="login-form-wrap">
      ...
      <form id="login-form" onsubmit="handleLogin(event)">
        ...
        <div class="login-fg">
          ...
          <div class="login-input-wrap">
            ...
            <button type="button" class="login-pw-toggle" onclick="togglePassword()">
        ...
        <div class="login-options">
          <label class="login-remember">
            <label class="tog-s"><input type="checkbox"/><div class="tog-track"></div><div class="tog-thumb"></div></label>
            Lembrar-me
          </label>
      ...
      <button class="login-btn-google">
        <svg viewBox="0 0 24 24" fill="none" style="width:18px;height:18px"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.65-2.07.01-.77z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
        Entrar com Google
      </button>

      <div class="login-footer">
        Nao tem uma conta? <a href="#">Solicitar acesso</a>
      </div>
...
function handleLogin(e){
  e.preventDefault();
  var btn=document.getElementById('btn-enter'),err=document.getElementById('login-error');
  ...
  setTimeout(function(){
    ...
    var em=document.getElementById('login-email').value,pw=document.getElementById('login-password').value;
    if(em.length>3&&pw.length>3){window.location.href='demo.php'}
    else{document.getElementById('login-error-msg').textContent='Credenciais invalidas. Verifique seu email e senha.';err.classList.add('show');var f=document.getElementById('login-form');f.style.animation='shake .4s ease';setTimeout(function(){f.style.animation=''},400)}
  },1500);
}
