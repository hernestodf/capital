...
if (!empty($unidadesMedida)) {
    foreach ($unidadesMedida as $um) {
        $selected = ($planilha['id_unidademedida'] ?? '') == $um['id'] ? 'selected' : '';
...
// Formatar valor para exibicao
$valorFormatado = '';
if (!empty($planilha['valor'])) {
    $valorFormatado = number_format($planilha['valor'], 2, ',', '.');
}
?>
    ...
    <section class="section active" id="sec-planilhas">
      <div class="section-header">
        ...
        <div>
          <div class="section-title">Editar Planilha</div>
          <div class="section-sub">Atualizar dados da planilha</div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <form method="POST" action="<?= $baseUrl ?>/planilhas/update/<?= $planilha['id'] ?>" data-ajax data-redirect="<?= $baseUrl ?>/planilhas">
            <input type="hidden" name="_csrf_token" value="<?= \App\Core\Csrf::getToken() ?>">

            <div class="fg">
              <div class="fl">Item *</div>
              <input type="text" name="item" class="fi" placeholder="Nome do item" value="<?= htmlspecialchars($planilha['item'] ?? '') ?>" required/>
            </div>

            <div class="fg" style="margin-top:16px">
              <div class="fl">Descricao</div>
              <textarea name="descricao" class="fi" rows="3" placeholder="Descricao detalhada do item"><?= htmlspecialchars($planilha['descricao'] ?? '') ?></textarea>
            </div>
            ...
            <div class="col2" style="margin-top:16px">
              ...
              <div class="fg">
                ...
                <input type="text" name="valor" id="valor" class="fi" placeholder="0,00" value="<?= htmlspecialchars($valorFormatado) ?>"/>
              </div>
            </div>
            ...
            <div class="col2" style="margin-top:16px">
              <div class="fg">
                ...
                <div class="fi-prefix-group">
                  ...
                  <input type="number" name="potencia_w" id="potencia_w" class="fi" min="0" step="1"
                         placeholder="Ex: 1500"
                         value="<?= $planilha['potencia_w'] > 0 ? (int)$planilha['potencia_w'] : '' ?>"/>
                </div>
              </div>
              <div class="fg">
                ...
                <div class="fi-prefix-group">
                  ...
                  <input type="number" name="horas_uso" class="fi" min="0" step="0.5"
                         placeholder="Ex: 20"
                         value="<?= $planilha['horas_uso'] > 0 ? $planilha['horas_uso'] + 0 : '' ?>"/>
                </div>
              </div>
            </div>
            ...
            <div style="margin-top:24px;display:flex;gap:8px">
              <button type="submit" class="btn btn-cyan">Atualizar Planilha</button>
