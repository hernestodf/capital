...
// ============ RENDER HTML (helper global) ============
// window.renderHTML() e chamado em ~60 pontos de public/js/eventos/*.js
// (rh.js, montar-os-serial.js, salas-produtos.js, fechamento*.js,
// alocacao-estoque.js) esperando que o framework reativo de src/js/ estivesse
// ativo e o definisse. Como src/js/ nunca chega a rodar em producao (ver
// memory/bugs.md bug-033), toda essa renderizacao dinamica lancava
// "TypeError: window.renderHTML is not a function". Implementacao replicada
// de src/js/utils/dom.js (mesma logica: <template> + replaceChildren, nao
// executa <script> embutido).
const _renderHtmlTemplate = document.createElement('template');
function parseHTML(html) {
  _renderHtmlTemplate.innerHTML = html;
  return _renderHtmlTemplate.content.cloneNode(true);
}
...
// ============ INPUT MASK (data-mask, delegado globalmente) ============
// A logica antiga (copiada em 8 views: usuario/colaborador/cliente/fornecedor
// create+edit) sempre concatenava os caracteres literais da mascara (parenteses,
// espaco, traco) mesmo depois de esgotar os digitos disponiveis -- digitar so
// "11" num campo com mascara "(XX) XXXX-XXXX" virava "(11) -" em vez de "(11".
// Alem disso, produtor/comprador (create+edit) tinham o atributo data-mask mas
// nenhum script pra aplica-lo -- mascara nunca funcionou ali. Um unico listener
// delegado em document (funciona em qualquer input[data-mask] presente ou
// futuro) resolve os dois problemas de uma vez.
function applyDataMask(value, mask) {
  const digits = value.replace(/\D/g, '');
  let result = '';
  let di = 0;
  for (const ch of mask) {
    if (di >= digits.length) break;
    if (ch === 'X') { result += digits[di]; di++; } else { result += ch; }
  }
  return result;
}
document.addEventListener('input', function(e) {
  const input = e.target;
  if (!input.dataset || !input.dataset.mask) return;
  input.value = applyDataMask(input.value, input.dataset.mask);
});
...
window.apiFetch = (url, options = {}) => {
  ...
  if (csrfToken) {
    if (data instanceof FormData) {
      ...
      if (data) {
        data += '&_csrf_token=' + encodeURIComponent(csrfToken);
      } else {
        data = '_csrf_token=' + encodeURIComponent(csrfToken);
      }
    }
...
function toggleSub(id, el) {
  ...
  document.querySelectorAll('.sub').forEach((s) => { s.classList.remove('open'); });
...
function showSection(id, el) {
  ...
  document.querySelectorAll('.ni,.si').forEach((n) => { n.classList.remove('active'); });
...
function openModal(id) {
  ...
  document.querySelectorAll('.popover.open').forEach((pop) => {
    ...
    const wrap = pop.closest('.popover-wrap');
...
function togglePop(id) {
  ...
  if (openPop && openPop !== el) {
    ...
    const prevWrap = openPop.closest('.popover-wrap');
  ...
  if (el.classList.contains('open')) {
    ...
    if (el.classList.contains('td-act-dropdown')) {
      const btn = el.parentElement.querySelector('.td-act-toggle');
...
window.addEventListener('scroll', () => {
  if (openPop && openPop.classList.contains('td-act-dropdown')) {
    const btn = openPop.parentElement.querySelector('.td-act-toggle');
    const r = btn.getBoundingClientRect();
...
function showToast(type, title, msg, duration) {
  ...
  t._timer = setTimeout(() => { dismissToast(t); }, duration || 4500);
}
...
function switchH(g, i, el, color) {
  ...
  parent.querySelectorAll('[class^="ht-"]').forEach((t) => {
    t.classList.remove('active');
    ['red','cyan','green','yellow','purple'].forEach((c) => { t.classList.remove(c); });
  });
  el.classList.add('active');
  if (color) el.classList.add(color);
  const pfx = { c: 'hc' }[g];
  parent.querySelectorAll('[class^="ht-"]').forEach((j) => {
    const p = document.getElementById(pfx + '-' + Array.from(parent.querySelectorAll('[class^="ht-"]')).indexOf(j));
    if (p) p.classList.remove('active');
  });
  const tgt = document.getElementById(pfx + '-' + i);
...
function updateCarousel() {
  ...
  document.querySelectorAll('#demo-carousel .carousel-dot').forEach((d, i) => { d.classList.toggle('active', i === carouselIndex); });
}
...
function selectChip(el) { const g = el.parentElement; g.querySelectorAll('.lc-chip').forEach((c) => { c.classList.remove('active'); }); el.classList.add('active'); }
...
function switchRcTab(id, el) {
  ...
  document.querySelectorAll('.rc-pane').forEach((p) => { p.classList.remove('active'); });
  el.classList.add('active');
...
function setRating(el, val) {
  const group = el.closest('.rating-group');
...
function hoverRating(el, val) {
  const group = el.closest('.rating-group');
  group.querySelectorAll('.star').forEach((s, i) => { s.classList.toggle('hover', i < val); });
}
function leaveRating(el) { el.closest('.rating-group').querySelectorAll('.star').forEach((s) => { s.classList.remove('hover'); }); }
...
function sortTable(colIdx, btn) {
  ...
  table.querySelectorAll('.th-sort').forEach((b) => { b.dataset.sort = ''; b.querySelector('.sort-ico').textContent = '↕'; });
  btn.dataset.sort = asc ? 'asc' : 'desc';
  btn.querySelector('.sort-ico').textContent = asc ? '↑' : '↓';
  rows.sort((a, b) => {
    const va = a.cells[colIdx].textContent.trim(), vb = b.cells[colIdx].textContent.trim();
...
function tblInit(id) {
  ...
  if (!table) return;
  const perPage = parseInt(table.dataset.perPage) || 5;
...
function tblApplyView(id) {
  ...
  if (state.searchTerm) {
    visibleRows = rows.filter((r) => {
      const text = normalizarTexto(r.textContent);
      return text.indexOf(state.searchTerm) !== -1;
    });
  }

  // Mark all rows hidden first
  rows.forEach((r) => { r.classList.add('tbl-hidden'); });

  // Calculate pagination
  const total = visibleRows.length;
  const isPaginated = table.dataset.paginated === 'true';
  const totalPages = isPaginated ? Math.max(1, Math.ceil(total / state.perPage)) : 1;
  if (state.page > totalPages) state.page = totalPages;

  const start = isPaginated ? (state.page - 1) * state.perPage : 0;
  const end = isPaginated ? start + state.perPage : total;
  const pageRows = visibleRows.slice(start, end);

  // Show visible rows
  pageRows.forEach((r) => { r.classList.remove('tbl-hidden'); });
  ...
  // Pagination buttons
  const pagesEl = document.getElementById(id + '-pages');
  ...
  const pagination = container ? container.querySelector('.tbl-pagination') : null;
  if (pagination) {
...
function loadSkeleton() {
  const btn = document.getElementById('skeleton-load-btn');
...
function switchTab(group, index) {
  ...
  document.querySelectorAll('[data-tab-group="' + group + '"][data-tab-index]').forEach((el) => {
    el.classList.remove('active');
  });
  // Activate clicked tab
  const tabs = document.querySelectorAll('[data-tab-group="' + group + '"][data-tab-index="' + index + '"]');
  tabs.forEach((el) => { el.classList.add('active'); });
}
 ...
 */
window.fetchAuthed = (url, options) => {
  options = options || {};
...
// ============ ACTION REGISTRY ============
// Global registry for data-action handlers (used by all JS files)
window._actionHandlers = {};
window.registerAction = function(name, handler) {
  window._actionHandlers[name] = handler;
};
window.registerActions = function(actions) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.body.addEventListener('click', (e) => {
    ...
    // Sidebar / Canvas
    if (action === 'open-left') { openLeft(); return; }
    if (action === 'close-left') { closeLeft(); return; }
    if (action === 'open-right') { openRight(); return; }
    if (action === 'close-right') { closeRight(); return; }
    ...
    if (action === 'open-modal') {
      ...
      if (target) { const m = document.getElementById(target); if (m) { m.style.display = 'flex'; m.classList.add('open'); } }
      return;
    }
    if (action === 'close-modal') {
      const target = el.dataset.target || el.dataset.modalId;
      if (target) { const m = document.getElementById(target); if (m) { m.style.display = 'none'; m.classList.remove('open'); } }
      return;
    }
    if (action === 'close-modal-overlay' || action === 'close-on-backdrop') {
      // So fecha quando o clique foi no proprio overlay (fundo), nao em um filho
      // sem data-action proprio que tenha feito o evento borbulhar ate aqui.
      if (el === e.target) {
        const target = el.dataset.modalId || el.dataset.target;
        if (target) { const m = document.getElementById(target); if (m) { m.style.display = 'none'; m.classList.remove('open'); } }
      }
      return;
    }

    // Elemento que so deve impedir o clique de borbulhar pro pai (ex.: label
    // de upload dentro de uma linha clicavel) — nao faz nenhuma outra acao.
    if (action === 'stop-propagation') { e.stopPropagation(); return; }

    // Chip selection
    if (action === 'select-chip') { selectChip(el); return; }

    // Popover/dropdown toggle
    if (action === 'toggle-pop') {
      const target = el.dataset.target || el.dataset.popId;
      if (target) togglePop(target);
      return;
    }

    // Registered handlers from other JS files
    const handler = window._actionHandlers[action];
    if (handler) {
      handler(el, e);
      return;
    }

    // Fallback: check window[actionName] functions
    const fnName = action.replace(/-([a-z])/g, (m, c) => c.toUpperCase());
    if (typeof window[fnName] === 'function') {
      window[fnName](el, e);
      return;
    }

    // Debug: log unhandled actions
    console.warn('[Capital] Unhandled data-action:', action, el);
  });

  initProgressBars();
  updateStepper();
  // Init all paginated/searchable tables
  document.querySelectorAll('table[data-paginated="true"]').forEach((t) => {
